...
void test_angle_consistancy(const std::vector<Edge>& input_edges, const std::vector<Edge>& output_edges) {
    if (input_edges.size() != output_edges.size()) {
        throw std::invalid_argument("Input and output edges must have the same size.");
    }

    for (int i = 0; i < input_edges.size(); ++i) {
        if (input_edges[i].node_0->node != output_edges[i].node_0->node ||
            input_edges[i].node_1->node != output_edges[i].node_1->node) {
            throw std::invalid_argument("Input and output edges must have the same nodes.");
        }
    }
}
...
Score calc_score(const std::vector<Node>& output_nodes,
    ...
    test_angle_consistancy(input_edges, output_edges);
